...
import { AlertCircle, Search, RefreshCw, FileSpreadsheet, Eye, X, Download, User } from 'lucide-react';
import { adminListBpjsRecords, adminExportBpjsRecords, adminGetUsers } from '../../lib/api';
import type { UserItem, BPJSRecord } from '../../lib/api';
import { toUserMessage } from '../../lib/errors';

export default function ListBPJSAdmin() {
  ...
  const [petugasList, setPetugasList] = useState<UserItem[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await adminListBpjsRecords({
        page,
        limit,
        search: activeSearch,
  ...
  }, [page, activeSearch, selectedCreatorId]);

  const fetchPetugas = useCallback(async () => {
    try {
      const res = await adminGetUsers({ role: 'petugas_bpjs', limit: 100 });
      setPetugasList(res.data);
    } catch (err) {
      console.error('Gagal memuat list petugas BPJS', err);
    }
  }, []);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await adminExportBpjsRecords({
        search: activeSearch,
        creatorId: selectedCreatorId
      });
    } catch (err: any) {
      setError(toUserMessage(err, 'Gagal mengekspor data BPJS'));
    } finally {
      setExporting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
        <div>
          <p className="text-sm font-semibold text-primary">BPJS Management</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Data BPJS</h1>
          <p className="mt-1 text-sm text-slate-500">Monitoring data kepesertaan BPJS dari semua petugas.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2">
            <button 
                onClick={handleExport} 
                disabled={exporting || data.length === 0}
                className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="p-4 border-b border-slate-200 flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto flex-1">
            ...
            <div className="relative w-full sm:w-64">
                ...
                <select
                    ...
                    className="block w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-primary focus:border-primary sm:text-sm appearance-none bg-white"
                >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 w-full lg:w-auto justify-center"
          >
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Diupload Oleh</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Status & Tgl Masuk</th>
              ...
              {loading && data.length === 0 ? (
                ...
                data.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-slate-900">{item.creator_name || '-'}</div>
                      <div className="text-xs text-slate-500">Petugas BPJS</div>
                    </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col cursor-default" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-white">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                ...
                <div>
                  <h4 className="text-sm font-medium text-slate-500">Diupload Oleh</h4>
                  <p className="mt-1 text-base text-slate-900">{selectedDetail.creator_name || '-'}</p>
                </div>
                <div>
